...
var regressionManifest = []struct {
	...
	{"D16", "test/conformance/rules_test.go", []string{"TestReviewRouteTotalityAndSelfExtinguish", "TestDeliveryRouteTotalityAndSelfExtinguish"},
		"the RULE-NATIVE review + delivery routes (the reshape replaced check_coherence): reviewer rejection RE-ENTERS DEVELOPMENT (07b spawns a developer, D16 — not verify→park), a terminated reviewer with no verdict parks (07d, fail-closed), and delivery opens the PR only on verify.result=pass AND review.verdict.0=approved AND openspec.validated present, else parks — the roll-up is rule conditions over harness facts, fail-closed, never a Go route-token"},
	{"SB5", "internal/tools/verifyartifact/verifyartifact_test.go", []string{"TestVerifyRetryStampsResultButDoesNotStopLoop"},
		"a transport-fault verify Retry verdict RE-RUNS (no StopLoop) rather than terminating — and because the rule-native delivery route keys on verify.result eq pass/fail, a \"retry\" value triggers neither delivery nor a park, so a single docker flake during the final cold verify never parks a good run (the reshape replaced the dev.verified chaining marker with the direct run-fact route)"},
}
