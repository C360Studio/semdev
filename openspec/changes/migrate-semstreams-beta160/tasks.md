...
- [ ] 1.1 `go get github.com/c360studio/semstreams@v1.0.0-beta.160 && go mod tidy`;
      commit the bump alone so the break inventory is reproducible
- [ ] 1.2 Enumerate every compile error (`go build ./... 2>&1`), classify each
...
- [ ] 1.3 Record any break the recon did not predict as a design note in this
      change before fixing it

## 2. graphown rewrite (D1, D2)

- [ ] 2.1 `contracts.go`: strip owner-claim/lease logic and commentary; keep
      ...
      struct
- [ ] 2.2 Replace `binding.go` with construction: one
...
- [ ] 2.3 `writer.go`: `Replace` → `Reconcile` (same group blast radius —
...
- [ ] 5.1 Rewrite the three Go `PortDefinition` sites (`internal/intake`,
...
- [ ] 5.2 `configs/semdev-bootstrap.json` + `configs/semdev-live-gemini.json`:
...
- [ ] 5.3 Services block: remove inner `name` fields (strict decode rejects
      unknown fields); TOOL stream `tool.>` → `tool.execute.>` +
      `tool.result.>`; bump top-level config `version`
- [ ] 5.4 Sweep journey-local/test configs for the same three surfaces (port
