...
- [ ] 2.2 `customizations.semdev` / convention reader for the few run fields (test command, tier split, secret refs) — not an environment DSL (SB2)
- [ ] 2.3 `internal/harness` reshaped: `Manifest` references the declared image + run fields (drop the toolchain-modeling fields it no longer owns); keep `AssessReadiness` tier logic
