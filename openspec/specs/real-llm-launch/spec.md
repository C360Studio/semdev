# real-llm-launch Specification

## Purpose
Defines how a paid real-LLM run is configured, gated, driven, monitored, and
recorded: the model-registry shape must match what the pinned framework
actually implements (with real prices so cost facts are true), a declared run
fails loud rather than silently skipping, the admitted ask must reach the
authoring model, the journey proves the arc on station outcomes rather than
scripted contracts, and every run — converged, parked, or aborted — lands in
the evidence ledger with its cost and monitoring records before any milestone
claim cites it. First discharged by the M1 run (2026-07-19).
## Requirements
