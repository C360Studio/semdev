...
- [x] 7.2 The bounded loop runs in-sandbox: apply_patch → `measure_task` (real, in-container) → floors on the real diff → harness-stamped outcome (G3). **Measure-in-container DONE (7B):** provision_sandbox leaves a WARM container Up (new `runspace.Sandboxes` seam, reaped by the runtime on Stop); `measure_task` swapped from the host `cliexec` runner to `cleanroom.Runner.Exec` over that warm sandbox (fails closed if none provisioned, SB5); `dev-from-task/05` triggers it on the developer terminal. Journey station 11 proves `measurement.result.0.passed=true` (RequestCount 9). **Floors-on-the-real-diff DONE (7C):** H1 presence floor added to `floors.CheckAll` (rejects `len(TargetFiles)>0 ∧ len(Files)==0` — closes the g4 all-absent-reads-green hole; red-first `TestPresenceFloor`); `check_floors` stamps the aggregate `floor.finding.<i>.rejected` + StopLoop; `measure_task` stamps a `dev.measure_done` LOOP marker (create_change precedent); `dev-from-task/06` chains check_floors off the measure loop via that marker (warn-free — NOT a #519 field-to-field trigger; per architect ruling the token handshake is unnecessary, loop-terminal chaining suffices). Journey station 12 proves `floor.finding.0.rejected=false` (RequestCount 10).
- [x] 7.3 Floors-GATE + budget/retry/escalate (7D). Architect-ruled design (supersedes the token-handshake blueprint, #519-blocked): a **`check_gate` tool** (`internal/tools/checkgate`; a rule firing on a LOOP entity cannot read RUN facts, and distinct-object budget counting can't be a `length_*`, and #519 blocks the scalar budget compare — all three force Go) reads `measurement.result.<i>.passed` + `floor.finding.<i>.rejected` + DISTINCT-object count of `task.attempt.<i>` (7B MEDIUM-1) vs `task.spec.<i>.budget`, **fails closed on missing facts → escalate** (a read fault is a retryable tool error, NOT a stamped decision), derives advance/retry/escalate (pure `checkgate.Decide`, pinned exhaustively offline `TestDecideRoutingTable`+`TestDecideBudgetBoundaryIsExact`), stamps `dev.gate.<i>.{decision,reason}` (G7 evidence) + the `dev.gate_decision` loop marker (both writer `gate-tools`, G3 — schema takes only `task_index`); `check_floors` now stamps `dev.floors_done` on its own loop (the deferred marker, `TestCheckFloorsStampsFloorsDoneMarkerOnLoopEvenWhenRejecting`); `dev-from-task/07` gate-trigger fires on the floors-loop marker → check_gate (`TestGateTriggerIsSelfExtinguishing`); three router rules (`08a/b/c`) fire on the check_gate loop on the single `dev.gate_decision` value (mutually exclusive literal `eq`, snapshot-race-immune, shared self-extinguishing `dev.routed` marker, `TestGateRoutersAreSelfExtinguishing`): advance→`dev.task_cleared.<i>` (verify hands off, g8), retry→re-dispatch Amelia (fresh chain, re-arms measure→floors→gate), escalate→`run.awaiting_human` park (third park realization). **⚠ SERIALIZATION INVARIANT pin (load-bearing, replaces the token's matched-attempt guard): `TestOnlySanctionedDeveloperSpawners` asserts ONLY dispatch-developer (04) + the gate's retry router (08b) spawn a `role=developer` loop.** Journey **station 13** proves `dev.gate.0.decision=advance` + `dev.task_cleared.0` (RequestCount 11, WARN-free); red-first exhaustion covered offline (`TestExecuteEscalateWhenBudgetExhausted`/`FailsClosedOnMissingMeasurement`/`CountsDistinctAttemptObjects`). New rules wired into `configs/semdev-bootstrap.json` (version bump 0.11.0→0.12.0).
