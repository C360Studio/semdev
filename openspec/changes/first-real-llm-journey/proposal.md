...
- New env-gated real-LLM journey variant in `test/e2e`: gated on `SEMDEV_REAL_LLM=1`; a declared run with `ANTHROPIC_API_KEY` unset FAILS loud (D4 posture: declared intent never silently skips). Boots the real runtime with `model_registry` patched to Anthropic's OpenAI-compatible endpoint (provider `openai`, url `https://api.anthropic.com/v1`, model `claude-opus-4-8`, `api_key_env`, `tool_format: openai`, pricing 5.00/25.00 per 1M so `agent.loop.cost-usd` stamps real cost, `max_output_tokens`, `request_timeout`). Drives the SAME fixture arc with eventual/relaxed assertions: any change slug, bounded retry/rejection re-entry tolerated, generous windows, a park is a LOUD failure naming the parked state; scrapes loop token/cost facts for the ledger record.
...
- NO new vocabulary predicates (G9). NO rule-pack changes: the authoring station stays `tool_choice=function` single-turn; the donor-style multi-turn author + author persona fragment (rule 02's DEFERRED note) is a NAMED follow-up justified by run-1 evidence, partially mitigated here via the reason channel. NO operator CLI driver (the `experiment.Launch` path stays M1). Mock ladder green unchanged.
