...
var regressionManifest = []struct {
	...
	{"dev-from-task", "test/conformance/rules_test.go", []string{"TestDevRewakeIsSelfExtinguishing", "TestProjectionSpawnIsSelfExtinguishing", "TestDevRewakeGatedOnProjection", "TestDispatchDeveloperIsSelfExtinguishing", "TestMeasureTriggerIsSelfExtinguishing", "TestFloorsTriggerIsSelfExtinguishing"},
		"the dev re-wake + projection + dispatch-developer + measure + floors publish_agent spawns are fired-once via self-extinguishing markers (run.dev_kickoff / run.projection_kickoff / dev.dispatched / dev.measured / dev.floors_dispatched); the dev re-wake is gated behind task.spec projection (Codex P1); measure fires only on the developer loop's SUCCESSFUL terminal and appends the per-task attempt counter; floors chains off the measure loop via the dev.measure_done loop marker (warn-free, not a #519 field-to-field trigger)"},
