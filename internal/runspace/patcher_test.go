...
func TestPatcherApplyChangesCheckout(t *testing.T) {
	...
	touched, err := p.Apply(context.Background(), run, diff)
	if err != nil {
...
func TestPatcherRejectsPathEscape(t *testing.T) {
	...
	if _, err := p.Apply(context.Background(), run, escape); err == nil {
...
func TestPatcherRejectsEmptyAndTargetless(t *testing.T) {
	...
	if _, err := p.Apply(context.Background(), run, "   \n"); err == nil {
		t.Error("empty diff must fail closed")
	}
	if _, err := p.Apply(context.Background(), run, "not a diff, no headers\n"); err == nil {
...
func TestPatcherFailsClosedWithoutCheckout(t *testing.T) {
	...
	diff := "--- a/f.txt\n+++ b/f.txt\n@@ -1 +1 @@\n-x\n+y\n"
	if _, err := p.Apply(context.Background(), "org.p.agent.chain.execution.no-such-run", diff); err == nil {
...
func TestPatcherReportsApplyConflict(t *testing.T) {
	...
	if _, err := p.Apply(context.Background(), run, stale); err == nil {
