...
- [ ] 1.2 RED: `TestGitHubChannelReadFiltersByCursor` — the GitHub `Read` maps each
  ...
  those with `id > cursor` + the max id as the next cursor; an empty cursor returns
  ALL; no attribution guard (a polled comment has one author = the principal).
...
- [ ] 3.1 RED: `TestPollerReadsAwaitingApprovalRunsAndReleasesGate` — a fake channel
  ...
  second poll (cursor advanced) does NOT re-read the same comment; a restart (empty
  cursor) re-reads + re-applies as a NO-OP (idempotent — `alreadyApproved`).
- [ ] 3.2 RED: `TestListRunsAwaitingApproval` — the resolver enumerates runs at
  ...
  prefix query, G2 — never a lifecycle write).
...
- [ ] 3.4 Implement the poller (interval loop, in-memory `map[ThreadRef]Cursor`,
  awaiting-approval enumeration, `Read`→`handleMessage`; a `Read`/enumerate error is
  logged + retried next tick, never blocks; stops on ctx cancel) + the `poll` config
  block (`enabled`, `interval` default 15s) + `ListRunsAwaitingApproval` on the
  resolver + the XOR wiring in Start.
