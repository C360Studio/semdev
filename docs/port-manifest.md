...
| T7 | Channel-agnostic human comms seam | commit 089a5eb5 front-door bus (`user.response.<loop_id>`), coordinator rules 03/03b | pattern | REALIZED as the `conversation-channel` capability (conversation-channel-seam): the `Channel` port (`Post`/`ResolveThread`) + neutral `Message` + the `conversation-channel` component (approval + park-post). v1 impl: GitHub issue/PR comments; Slack/Jira later behind the same port without touching the arc. NOTE: `github_list_comments` (LLM-grounding thread reader) stays a legitimately host-specific code-host read tool, NOT subsumed by the port (L-1). |
