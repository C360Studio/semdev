...
- [x] 7.0 ARCHIVE NOTE (2026-07-19): archived SOLO pre-merge, resolving 7.2/7.3's deferred decision the other way — this change carries no spec deltas, all its tasks closed with 7.1 below, and the sibling changes' deltas were synced to `openspec/specs/` the same day (the branch's spec truth no longer depends on a joint archive)
- [x] 7.1 Re-project the change specs (predicate renames across forge-io/run-lifecycle/dev-from-task/harness-measurement/openspec-io/clean-room-verify/sandbox — these live in the sibling changes' `specs/` on this draft branch; G10 hygiene). DONE: swept all sibling delta specs for pre-beta.147 names (underscore forms, `agent.run`, `rule.spawned_task`, 4-seg `openspec.spec.*`) — the only remnants were the two openspec-io deltas' `openspec.spec.*` tree wording, re-projected to the canonical `openspec.spec.document` blob
