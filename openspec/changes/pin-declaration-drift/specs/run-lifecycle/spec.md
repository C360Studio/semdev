# run-lifecycle — delta for pin-declaration-drift
...
Clean-room verification that finds the committed declaration diverged from the
one the provisioning baseline attested SHALL likewise cause the run to park: the
verify harness records the drift on the run (single writer, the tool that
performed the comparison; the object names what changed), stamps no verdict, and
a park rule records `run.awaiting.human` from that fact. Drift is neither a pass
nor a failure — the artifact was never measured under the contract that was
proven — so no rule SHALL route it as an outcome, and no rule SHALL re-contract
the run onto the new declaration without a human signal.
...
#### Scenario: Declaration drift parks the run
- **WHEN** the declaration resolved from the committed snapshot differs from the attested one
- **THEN** the verify harness records the drift and records no verdict
- **AND** a rule records `run.awaiting.human` on the run, naming what changed
- **AND** no `verify.cleanroom.result` and no `delivery.pr.ref` ever appear on that run

#### Scenario: A drifted run is not auto-accepted onto the new declaration
- **WHEN** a run is parked on declaration drift
- **THEN** no rule re-proves the baseline against the new declaration and resumes
- **AND** the run resumes only on a human signal
