...
// newExec builds an executor over the fakes with the real docker probe bypassed.
func newExec(sources Sources, checkouts Checkouts, manifests Manifests, prover Prover, reader fakeReader, w *fakeWriter) *Executor {
	e := New(sources, checkouts, manifests, prover, nil, reader, w, nil)
...
func TestReadyStampsAttestationPackage(t *testing.T) {
	...
	e := newExec(fakeSources{dir: "/src"}, &fakeCheckouts{root: "/checkout"}, fakeManifests{m: harness.GoProfile()}, prover, fakeReader{}, w)
...
func TestBaselineNotReadyBlocks(t *testing.T) {
	...
	e := newExec(fakeSources{dir: "/src"}, &fakeCheckouts{root: "/checkout"}, fakeManifests{m: harness.GoProfile()}, &fakeProver{baseline: notReady}, fakeReader{}, w)
...
func TestRetryBaselineBlocksTowardHuman(t *testing.T) {
	...
	e := newExec(fakeSources{dir: "/src"}, &fakeCheckouts{root: "/checkout"}, fakeManifests{m: harness.GoProfile()}, &fakeProver{baseline: retry}, fakeReader{}, w)
...
func TestProverErrorBlocks(t *testing.T) {
	w := &fakeWriter{}
	e := newExec(fakeSources{dir: "/src"}, &fakeCheckouts{root: "/checkout"}, fakeManifests{m: harness.GoProfile()}, &fakeProver{err: errors.New("declare an image")}, fakeReader{}, w)
...
func TestReadyBaselineButNoSandboxTierBlocks(t *testing.T) {
	...
	e := newExec(fakeSources{dir: "/src"}, &fakeCheckouts{root: "/checkout"}, fakeManifests{m: m}, &fakeProver{baseline: readyBaseline("sha256:x")}, fakeReader{}, w)
...
func TestDockerAbsentBlocks(t *testing.T) {
	...
	e := New(fakeSources{dir: "/src"}, checkouts, fakeManifests{m: harness.GoProfile()}, prover, nil, fakeReader{}, w, nil)
...
func TestSourceResolveFailureBlocks(t *testing.T) {
	w := &fakeWriter{}
	checkouts := &fakeCheckouts{root: "/checkout"}
	e := newExec(fakeSources{err: errors.New("no source configured")}, checkouts, fakeManifests{m: harness.GoProfile()}, &fakeProver{}, fakeReader{}, w)
...
func TestAlreadyReadyIsNoOp(t *testing.T) {
	...
	e := newExec(fakeSources{dir: "/src"}, checkouts, fakeManifests{m: harness.GoProfile()}, prover, reader, w)
...
func TestFailsLoudlyWithoutHarness(t *testing.T) {
	res, err := New(nil, nil, nil, nil, nil, nil, nil, nil).Execute(context.Background(), call())
	if err != nil {
		t.Fatalf("execute: %v", err)
	}
	if res.Error == "" {
...
func TestMissingRunEntityFailsLoudly(t *testing.T) {
	w := &fakeWriter{}
	e := newExec(fakeSources{dir: "/src"}, &fakeCheckouts{root: "/checkout"}, fakeManifests{m: harness.GoProfile()}, &fakeProver{}, fakeReader{}, w)
...
func TestProvisionRealFixtureReady(t *testing.T) {
	...
	w := &fakeWriter{}
	e := New(fakeSources{dir: fixtureDir(t)}, checkouts, runspace.Manifests{}, DefaultProver(), nil, fakeReader{}, w, nil)
