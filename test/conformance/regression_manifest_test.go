...
var regressionManifest = []struct {
	...
	{"dev-from-task", "test/conformance/rules_test.go", []string{"TestDevRewakeIsSelfExtinguishing", "TestProjectionSpawnIsSelfExtinguishing", "TestDevRewakeGatedOnProjection", "TestDispatchDeveloperIsSelfExtinguishing", "TestMeasureTriggerIsSelfExtinguishing", "TestFloorsTriggerIsSelfExtinguishing", "TestGateTriggerIsSelfExtinguishing", "TestGateRoutersAreSelfExtinguishing", "TestOnlySanctionedDeveloperSpawners", "TestReviewTriggerIsSelfExtinguishing", "TestVerifyTriggerIsSelfExtinguishing", "TestCoherenceTriggerIsSelfExtinguishing", "TestDeliveryRoutersAreSelfExtinguishing", "TestOnlySanctionedOpenPRSpawners", "TestOnlySanctionedParkWriters"},
	...
	{"D16", "internal/tools/checkcoherence/checkcoherence_test.go", []string{"TestDecideRollup", "TestExecuteBlocksWhenProjectedTaskUnreviewed", "TestExecuteBlocksOnFailingVerify"},
		"the open_pr coherence gate rolls up verify.result + openspec.validated + every projected review.verdict and FAILS CLOSED — a non-pass/absent verify, an unvalidated change, no projected task, or ANY projected task without an approved verdict blocks delivery (an absent verdict is not approval, D16); a missing signal never opens a PR (semspec encoded couldnt-prove as a pass)"},
	{"SB5", "internal/tools/verifyartifact/verifyartifact_test.go", []string{"TestVerifyRetryStampsResultButDoesNotChainOrStopLoop"},
		"a transport-fault verify Retry verdict RE-RUNS (no dev.verified marker, no StopLoop) rather than chaining forward — so a single docker flake during the final cold verify never parks a good run (the whole point of the Retry classification: never terminally reject a good artifact on a flake)"},
}
