# run-lifecycle — delta for sealed-run-contract
...
A run SHALL also park toward the human when its contract cannot be established
or has been disturbed: when the target repo declares no profile, when the declared
profile disagrees with the repo's markers, and when the sealed declaration surface
changed during the run. Each SHALL be recorded by the harness that detected it and
routed by a park rule, and none SHALL be auto-reconciled — no rule SHALL adopt a
new declaration, re-resolve a contract, or re-prove a baseline in order to clear
one of these parks. They resume only on a human signal.
...
#### Scenario: An unestablished or disturbed contract parks the run
- **WHEN** the repo declares no profile, or the declaration disagrees with the
  repo's markers, or the sealed declaration surface changed during the run
- **THEN** the run parks toward the human naming which of these occurred
- **AND** no `verify.cleanroom.result` and no `delivery.pr.ref` ever appear on that run

#### Scenario: A contract park is never auto-reconciled
- **WHEN** a run is parked on a contract condition
- **THEN** no rule adopts the new declaration, re-resolves the contract, or re-proves the baseline
