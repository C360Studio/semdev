...
var regressionManifest = []struct {
	...
	{"dev-from-task", "test/conformance/rules_test.go", []string{"TestDevRewakeIsSelfExtinguishing", "TestProjectionSpawnIsSelfExtinguishing", "TestDevRewakeGatedOnProjection", "TestDispatchDeveloperIsSelfExtinguishing"},
		"the dev re-wake + projection + dispatch-developer publish_agent spawns are fired-once via self-extinguishing markers (run.dev_kickoff / run.projection_kickoff / dev.dispatched); the dev re-wake is gated behind task.spec projection so approval freezes the immutable task surface before the loop routes into development (Codex P1, restart-safety)"},
