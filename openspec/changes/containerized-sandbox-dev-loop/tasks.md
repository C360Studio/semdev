...
- [ ] 7.3 Floors-GATE + budget/retry/escalate (7D). Architect-ruled design (supersedes the token-handshake blueprint, #519-blocked): a **`check_gate` tool** (a rule firing on a LOOP entity cannot read RUN facts, and distinct-object budget counting can't be a `length_*`, and #519 blocks the scalar budget compare — all three force Go) reads `measurement.result.<i>.passed` + `floor.finding.<i>.rejected` + DISTINCT-object count of `task.attempt.<i>` (7B MEDIUM-1) vs `task.spec.<i>.budget`, fails closed on missing facts / ambiguity → escalate, derives advance/retry/escalate, stamps `dev.gate.<i>.decision` + a loop-marker; `check_floors` stamps `dev.floors_done` on its loop; `dev-from-task/07` gate-trigger fires on the floors-loop marker → check_gate; three router rules (`08a/b/c`) fire on the check_gate loop on the single `dev.gate_decision` value (mutually exclusive literal `eq`, snapshot-race-immune): advance→g8, retry→re-dispatch Amelia (fresh chain), escalate→`run.awaiting_human` park. **⚠ NEW conformance pin needed: "one developer in flight; only dispatch-developer + the gate's retry branch may spawn a role=developer loop" — the serialization invariant is now load-bearing for correctness (it replaces the token's matched-attempt guard).** Station 13 + red-first exhaustion pin.
